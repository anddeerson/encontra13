...
def extrair_texto_pdf(pdf_file):
    """Extrai texto de PDFs baseados em texto."""
...
def extrair_nomes_pdf(pdf_file):
    ...
    matches = re.findall(r'\b[A-ZÀ-Ú][A-ZÀ-Ú]+\s[A-ZÀ-Ú ]+\b', text)  # Expressão regular para capturar nomes completos
...
st.title("Encontra aluno(s). Versão 1.3 - Busca pelo nome completo - Sem Scan por OCR")

names_input = st.text_area("Cole a lista de nomes (um por linha):")
names = [name.strip() for name in names_input.split("\n") if name.strip()]

pdf_files = st.file_uploader("Anexe o(s) PDF(s)", type="pdf", accept_multiple_files=True)
